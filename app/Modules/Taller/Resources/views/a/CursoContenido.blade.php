                        ...
                        <div class="list-group list-group-flush" style="max-height: 500px; overflow-y: auto;">
                            @forelse($curso->contenidos as $contenido)
                                @php
                                    ...
                                    $icono = 'fa-file-alt';
                                    if(strtolower($contenido->tipo_contenido) == 'video') $icono = 'fa-play-circle';
                                    if(strtolower($contenido->tipo_contenido) == 'archivo') $icono = 'fa-download';
                                    if(strtolower($contenido->tipo_contenido) == 'enlace') $icono = 'fa-link';
                                @endphp
                                   ...
                                   class="list-group-item list-group-item-action d-flex align-items-center p-3 {{ $esActivo ? 'active bg-primary text-white border-primary' : '' }}">
                                    ...
                                    <div class="flex-grow-1">
                                        <div class="d-flex justify-content-between align-items-center">
                                            <h6 class="mb-0 {{ $esActivo ? 'text-white' : '' }}">{{ $contenido->titulo }}</h6>
                                            <small class="{{ $esActivo ? 'text-white-50' : 'text-muted' }} ms-2">
                                                {{ $contenido->orden }}
                                            </small>
                                        </div>
                                        <small class="{{ $esActivo ? 'text-white-50' : 'text-muted' }} d-block text-truncate" style="max-width: 250px;">
            ...
            <div class="col-lg-8 order-lg-1">
                        ...
                        <div class="card-body p-4">
                            ...
                            <div class="d-flex flex-wrap align-items-center justify-content-between mb-4 pb-3 border-bottom">
                                <div class="d-flex align-items-center mb-2 mb-md-0">
                                    <span class="badge bg-light text-dark border me-3 px-3 py-2 rounded-pill fw-normal">{{ ucfirst($tipo) }}</span>
                                    <span class="text-muted small"><i class="far fa-calendar-alt me-1"></i> {{ $contenidoActual->created_at ? $contenidoActual->created_at->format('d/m/Y') : 'N/A' }}</span>
                                </div>
                                <a href="{{ $url }}" target="_blank" class="btn btn-outline-dark rounded-pill px-4 btn-action">
                            ...
                            <div class="contenido-descripcion mt-4">
                                <h5 class="text-dark fw-bold mb-3" style="font-family: 'Poppins', sans-serif;">Sobre este contenido</h5>
                ...
                @else
                    <div class="empty-state text-center py-5">
                        <div class="empty-icon bg-light rounded-circle mx-auto mb-4 d-flex align-items-center justify-content-center" style="width: 100px; height: 100px;">
                            <i class="fas fa-play fa-2x text-muted"></i>
                        </div>
                        <h4 class="fw-bold text-dark">Selecciona un contenido</h4>
                        <p class="text-muted text-center" style="max-width: 400px; margin: 0 auto;">Elige una lección del menú de navegación para comenzar tu aprendizaje.</p>
                    </div>
                @endif
            </div>
        </div>
    ...
    @push('styles')
        <style>
            ...
            .list-group-item.active {
                ...
                color: #2dce89 !important; /* Color primario suave o verde éxito */
                font-weight: 600;
                border: 1px solid #e9ecef;
                box-shadow: inset 4px 0 0 #2dce89; /* Borde izquierdo activo */
            }
